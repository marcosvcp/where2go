...
/**
 * Representa o estado de um {@link Invitation} pendente
...
public class InvitationPending implements InvitationState {
    ...
    public Notification confirm(Invitation invitation) {
        if (invitation.getEvent().addParticipant(invitation.getGuest())) {
            ...
            return new Notification(invitation.getGuest(),
                    invitation.getEvent(), String.format(EVENT_MSG, invitation.getEvent().getName(), "confirmed"));
        }
        return new Notification(invitation.getGuest(), invitation.getEvent(), "Sorry, but you don't have access to this event");
    }
    ...
    public Notification decline(Invitation invitation) {
        ...
        return new Notification(invitation.getGuest(),
                invitation.getEvent(), String.format(EVENT_MSG, invitation.getEvent().getName(), "declined"));
    }
}
